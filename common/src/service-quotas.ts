...
export const UpsertQuotas = async (
    ...
    for (const [key, val] of Object.entries(desired)) {
        ...
        if (prevVal !== val) {
            if (prevVal && prevVal > val) {
                throw new Error(
                    `Decrease of limit failed because desired value ${val} is lower than previous value ${prevVal}`
                );
            }
            ...
            try {
                ...
                if (history.RequestedQuotas && history.RequestedQuotas.length > 0) {
                    ...
                    if (lastRequest && lastRequest.DesiredValue > val) {
                        throw new Error(
                            `Decrease of limit failed because desired value ${val} is lower than last requested value ${lastRequest.DesiredValue}`
                        );
                    } else if (lastRequest && lastRequest.DesiredValue == val) {
                        logger.info(
                            `skipping update because desired value ${val} is equal to last requested value ${lastRequest.DesiredValue}`
                        );
                        return;
                    }
                }
            ...
            try {
                ...
                if (quotaResponse.Quota && quotaResponse.Quota.Value > val) {
                    throw new Error(
                        `Decrease of limit failed because desired value ${val} is lower than current quota value ${quotaResponse.Quota.Value}`
                    );
                } else if (quotaResponse.Quota && quotaResponse.Quota.Value == val) {
                    logger.info(
                        `skipping update because desired value ${val} is equal to current quota value ${quotaResponse.Quota.Value}`
                    );
                    return;
                }
            } catch (err) {
                if (err && err.code === 'NoSuchResourceException') {
                    const defaultQuota = await service
                        .getAWSDefaultServiceQuota(quota)
                        .promise();
                    if (defaultQuota.Quota && defaultQuota.Quota.Value > val) {
                        throw new Error(
                            `Decrease of limit failed because desired value ${val} is lower than AWS default quota value ${defaultQuota.Quota.Value}`
                        );
                    } else if (defaultQuota.Quota && defaultQuota.Quota.Value == val) {
                        logger.info(
                            `skipping update because desired value ${val} is equal to AWS default quota value ${defaultQuota.Quota.Value}`
                        );
                        return;
                    }
                } else {
            ...
            try {
                const response = await service
                    .requestServiceQuotaIncrease(increaseRequest)
                    .promise();

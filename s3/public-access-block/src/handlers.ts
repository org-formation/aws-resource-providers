...
const upsertAccountPublicAccessBlockHandler: ResourceProviderHandler<
    S3Control,
    ResourceModel
> = async (
    action: Action,
...
const deletePublicAccountBlockHandler: ResourceProviderHandler<
    S3Control,
    ResourceModel
> = async (action: Action, args: HandlerArgs, service: S3Control): Promise<null> => {
...
const readPublicAccountBlockHandler: ResourceProviderHandler<
    S3Control,
    ResourceModel
> = async (
    action: Action,
...
resource.addHandler(
    Action.Update,
    WrapHandler(Action.Update, 'S3Control', upsertAccountPublicAccessBlockHandler)
);
resource.addHandler(
    Action.Delete,
    WrapHandler(Action.Delete, 'S3Control', deletePublicAccountBlockHandler)
);
resource.addHandler(
    Action.Read,
    WrapHandler(Action.Read, 'S3Control', readPublicAccountBlockHandler)
);

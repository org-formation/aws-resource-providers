...
export class ResourceModel extends BaseModel {
    ...
    @Exclude()
    public static readonly TYPE_NAME: string = 'Community::SSM::PublicAccessBlock';

    @Exclude()
    protected readonly IDENTIFIER_KEY_RESOURCEID: string = '/properties/ResourceId';

    @Expose({ name: 'BlockPublicSharing' })
    @Transform(
        (value: any, obj: any) =>
            transformValue(Boolean, 'blockPublicSharing', value, obj, []),
        {
            toClassOnly: true,
        }
    )
    blockPublicSharing?: Optional<boolean>;
